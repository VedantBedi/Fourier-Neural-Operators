...
# =====================================================
#           FNO2D Model for RGB Output
# =====================================================
class FNO2D(nn.Module):
    def __init__(self, modes1, modes2, width, out_channels=3):
        super().__init__()
        self.fc0 = nn.Linear(5, width)  # 3 image channels + 2 coordinates
...
# =====================================================
#          FNO-based RGB Super-Resolution
# =====================================================
def fno_super_resolve_rgb(
    image,
    pad_shape=(64, 64),
    ...
    # Upsample low-res image directly instead of zero-padding
    image = image.resize(pad_shape, Image.LANCZOS)
    ...
    # No zero-padding applied here
    input_tensor = ToTensor()(image).unsqueeze(0).to(torch.float32).to(device)
    ...
    if plot:
        ...
        plt.subplot(1, 3, 2)
        plt.title("Upsampled Input (64x64)")
        plt.imshow(image)
...
if __name__ == "__main__":
    original = Image.open("image2.jpg").convert("RGB")
    resized = original.resize((32, 32))

    output = fno_super_resolve_rgb(
        image=resized,
        pad_shape=(64, 64),            # Resizes the low-res input to 64x64 before inference
        target_resolution=(64, 64),    # Target resolution for the output
        epochs=800
    )